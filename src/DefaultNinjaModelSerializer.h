...
public:
  ...
  {
    model.SetPoint = 8.0; // target beer temp
    model.Output = 8.0; // main PID output temperature, also setpoint for heatPID
    model.PeakEstimator = 5.0;
    model.PID_Kp = 3.0;
    model.PID_Ki = 5.0E-4;
    model.PID_Kd = 0.0;
    ...
    model.IdleDiff = 1;     // constrain fridge temperature to +/- 0.5 deg C (0.9 deg F) differential
    ...
    model.CoolMaxOn = 7200;     // maximum compressor on time, seconds (45 min)
